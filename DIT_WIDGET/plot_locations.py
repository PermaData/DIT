...
def plot_coords(lats, lons, lcx, lcy, ucx, ucy, dx ,dy):
    map = Basemap(projection='merc', lat_0 = 57, lon_0 = -135, resolution = 'l', area_thresh = 0.1, llcrnrlon=lcx, llcrnrlat=lcy, urcrnrlon=ucx, urcrnrlat=ucy) 
    map.drawcoastlines()
    map.drawcountries()
...
def degree2latlon(deg,min,sec,dir):
    ...
    return (int(deg)+int(min)/60.0+int(sec)/3600.0) * direction[new_dir]
...
plt.savefig('C:\Users\elchin\Documents\Projects\PermData\FGDC_to_GTNP\PD_manuscript\Figures\Schefferville_loc.png', transparent=True, bbox_inches='tight')
...
for i in range(n):
    str_N=str(deg_N[i])
    lats[i]=degree2latlon(int(str_N[0:2]),int(str_N[2:4]), int(str_N[4:6]), 'N')
    str_E=str(deg_E[i])
    lons[i]=degree2latlon(int(str_E[0:2]),int(str_E[2:4]), int(str_E[4:6]), 'E')
...
def string2degree(str_d):
    ...
    sdd=str_d[0:i1]; smm=str_d[i1+1:i2]; sss=str_d[i2+1:nn]
    #print sdd, smm, sss
    if sdd=='':
        sdd='0'
    if smm=='':
