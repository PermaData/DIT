...
def chk_count_distinct(input_data_file=None, output_data_file=None, log_file=None):
...
# count number records for each value
        logger.info('{:>5} {:>40} {:>10} {:>10}'.format('Num','Distinct Value','number','Percent'))	       
        count=0
        for value in distinct_values:
            count=count+1
            number=original_values.count(value)
            num_fraction=float(number)/float(record)*100
            logger.info('{:5.0f} {:>40} {:10.0f} {:10.3f}'.format(count,value,number,num_fraction))	       
