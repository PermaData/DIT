#!/usr/bin/python
"""Divide two columns (out = column_a / column_b)"""
...
def calc_divide_col(missing_value, input_data_file=None, output_data_file=None, log_file=None):
    ...
    with open(input_data_file, newline='') as _in, \
        ...
        for line in reader:
            new_line = array('f')
            if (float(line[0]) != float(missing_value)) and (float(line[1]) != float(missing_value)) and (float(line[1]) != 0.0):
...
def parse_arguments():
    parser = ap.ArgumentParser(description='Divide two columns (out = column_a / column_b).')
...
if __name__ == '__main__':
    ...
    calc_divide_col(args.missing_value, args.input_data_file, args.output_data_file, args.log_file)
