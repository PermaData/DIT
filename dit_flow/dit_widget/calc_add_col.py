#!/usr/bin/python
"""Add two columns (out = column_a +column_b)"""
...
def calc_add_col(missing_value, input_data_file=None, output_data_file=None, log_file=None, log_level=DEFAULT_LOG_LEVEL):
    ...
    with open(input_data_file, newline='') as _in, \
            ...
            logger.info('Add two columns (out = column_a +column_b)')
            output = csv.writer(_out)
            reader = csv.reader(_in)
            for line in reader:
                new_line = array('f')
                if (float(line[0]) != float(missing_value)) and \
                   (float(line[1]) != float(missing_value)):
...
if __name__ == '__main__':
    ...
    calc_add_col(args.missing_value, args.input_data_file, args.output_data_file, args.log_file)
