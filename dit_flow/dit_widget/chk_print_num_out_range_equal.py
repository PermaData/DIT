...
def chk_print_num_out_range_equal(startval, endval, input_data_file=None, output_data_file=None, log_file=None):
    ...
    with open(input_data_file, newline='') as _in:
        ...
        logger.info('\n\t Total number <= {} or >= {}: {:10.0f}'.format(startval,endval,float(count)))


def parse_arguments():
    parser = ap.ArgumentParser(description="Prints values <= startval or values >= endval in input_data_file.")
