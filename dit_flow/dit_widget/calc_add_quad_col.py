    ...
    with open(input_data_file, newline='') as _in, \
        ...
        for line in reader:
            ...
            if (float(line[0]) != float(missing_value)) and (float(line[1]) != float(missing_value)):
                value = math.sqrt(float(line[0])*float(line[0]) + float(line[1])*float(line[1]))
            else:
...
def parse_arguments():
    parser = ap.ArgumentParser(description='Add two columns in quadrature (out = sqrt(column_a**2 +column_b**2)).')
...
if __name__ == '__main__':
    ...
    calc_add_quad_col(args.missing_value, args.input_data_file, args.output_data_file, args.log_file)
