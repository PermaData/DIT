#!/usr/bin/python
"""If col A not = criteria, column B = constant."""
...
def cond_if_not_equal(criteria, constant, print_flag, input_data_file=None,
...
# If column A /= criteria in input_data_file column B = constant in output_data_file.
    ...
    with open(input_data_file, newline='') as _in, \
            ...
            logger.info('If column A  not = {} set column B to {}'.format(criteria, constant))
            if print_flag:
...
# perform conditional check
        ...
        for i, line in enumerate(original_values):
            ...
            if (str(col_a_value) != str(criteria)):
                count = count+1
...
def parse_arguments():
    parser = ap.ArgumentParser(description="If col A not = criteria, column B = constant.")
...
if __name__ == '__main__':
    ...
    cond_if_not_equal(args.criteria, args.constant, args.print_flag,
