...
class RunFlow():
    ...
    def __init__(self, flow_name, log_file=None):
        ...
        self.log_file = self.config_translator.get_log_file()

        if log_file is not None:
            self.log_file = log_file
        elif self.log_file is None:
            self.log_file = os.path.join(os.getcwd(), self.flow_name.split('.')[0] + '.log')
        self.log_level = self.config_translator.get_log_level()
        self.logger = setup_logger('', log_file=self.log_file, log_level=self.log_level)
        self.logger.debug('Setup logging into: {} with level: {}'.format(self.log_file, self.log_level))
    ...
    def setup_utilities(self):
        self.logger.debug('Setting up file manager widget')
    ...
    def setup_widget_list(self, widget_defns):
        widget_list = []
        self.logger.debug('Widgets to be run:')
        if widget_defns is None:
            self.logger.debug('\tNo widgets to run.')
            return widget_list
    ...
    def can_do_subset(self, np_data, input_columns):
        ...
        if np_data.size == 0:
            self.logger.debug('Cannot subset or replace, data is empty.')
            can_do_subset_replace = False
        if not ManipulationWidget.columns_exist(input_columns):
            self.logger.debug('Cannot subset, widget input columns missing.')
            can_do_subset_replace = False
    ...
    def can_do_replace(self, np_data, output_columns):
        ...
        if not ManipulationWidget.columns_exist(output_columns):
            self.logger.debug('Cannot replace, widget output columns missing.')
            can_do_subset_replace = False

        return can_do_subset_replace

    def read_input_data(self, input_file, log_file, log_level):
        data = self.file_reader.go(input_file, log_file=log_file, log_level=log_level)
    ...
    def subset_data(self, np_data, columns, with_header=False):
        ...
        if with_header:
            self.logger.debug('Subsetting columns with headers: {}'.format(columns))
        else:
            self.logger.debug('Subsetting columns without headers: {}'.format(columns))
    ...
    def set_widget_required_args(self, widget, widget_data_in_file, widget_data_out_file, log_file, log_level):
        ...
        widget.set_required_arg('log_level', log_level)
        return widget

    def format_to_output_data(self, input_data, variable_map, log_file, log_level):
        output_data = self.variable_mapper.go(input_data.tolist(), variable_map, log_file, log_level)
        return np.array(output_data, dtype=object)

    def do_manipulations(self, manipulations, np_data, output_dir, file_id, log_file, log_level):
        for step_id, widget in enumerate(manipulations, 1):
            if not widget.do_it:
                self.logger.debug('Not running widget: ' + widget.widget_name)
                continue
            try:
                ...
                widget = self.set_widget_required_args(widget, widget_data_in_file, widget_data_out_file, log_file, log_level)
                ...
                if self.can_do_subset(np_data, widget.input_columns):
                    ...
                    self.write_output_file(widget_data_in_file, widget_data, log_file, log_level)
                ...
                if self.can_do_replace(np_data, widget.output_columns):
                    ...
                    manipulated_data = self.read_input_data(widget_data_out_file, log_file, log_level)
    ...
    def write_output_file(self, output_file, np_data, log_file, log_level):
        self.file_writer.go(output_file, np_data, log_file=log_file, log_level=log_level)

    def print_file_delineator(self, file_name):
        seperator = '----------------------------------------------------------------'
    ...
    def run(self):
        ...
        files_n_ids = self.file_manager.go(self.input_files, output_dir, temp_dir, self.log_file, self.log_level)
        for input_file, output_file, file_id, log_file in files_n_ids:
            self.print_file_delineator(input_file)
            input_data = self.read_input_data(input_file, self.log_file, self.log_level)
            self.logger.debug('Doing input manipulations')
            self.do_manipulations(self.input_manipulations, input_data, temp_dir, file_id, self.log_file, self.log_level)
            ...
            self.do_manipulations(self.output_manipulations, output_data, temp_dir, file_id, self.log_file, self.log_level)

            self.logger.debug('Writing output file.')
            self.write_output_file(output_file, output_data, self.log_file, self.log_level)
