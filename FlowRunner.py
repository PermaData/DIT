...
from circuits import Worker, task, Event, Loader, Manager


def run_flow(flowname):
    m = Manager()
    worker = Worker().register(m)
    loader = WidgetLoader()
    loader.register(m)

    m.start()

    fileManager = loader.load("file_manager")

    print("fileManager loaded: ", fileManager)

    res = m.fire(task(fileManager._say_hi))
